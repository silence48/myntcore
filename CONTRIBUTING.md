...
We're working hard to make *myntcore* the most powerful JavaScript library for working with myntcoin. Our goal is to have *myntcore* be a library that can be used by anyone interested in myntcoin, and to level expertise differences with great design and documentation.
...
`JSON.stringify` and `JSON.parse` are expected to be handled outside of the scope of Myntore methods. For example, calling `JSON.stringify` on an MMyntre object will behave as expected and call `transaction.toJSON()` and then stringify it:
...
```sh
git remote add under git@github.com:silence48/myntcore.git
...
```
Finally go to [github.com/silence48/myntcore](https://github.com/silence48/myntcore) in your web browser and issue a new pull request.
...
[github.com/silence48/myntcore/issues](https://github.com/silence48/myntcore/issues).
